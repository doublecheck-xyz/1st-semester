...
if [ $1 == "create" ]; then
        ...
        cp generic_mln.template $project.mln
...
#       echo "Number of machines that failed:   $(nova list |grep $project | grep -c ERROR)"
        ...
        while [ $(/root/project2/cert_management.sh list | grep -c $project) -lt  $(nova list |grep -i $project | grep -c ACTIVE) ]
        do
                sleep 2
        done
        /root/project2/cert_management.sh sign $project no-confirm >> $project.log
        echo "Configuring backup of servers in the project"
        /root/project2/configure_backup.sh $project >> $project.log
